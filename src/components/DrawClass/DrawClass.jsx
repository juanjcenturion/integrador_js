...
class DrawClass extends Component {
    render() {
        const { name, attributes, methods, borderColor, headColor, textColor } =
            this.props.data;
        ...
        // Definir alturas
        const rectAttributeHeight =
            attributes.length > 1 ? attributes.length * 40 : 40; // 100 es el valor mínimo por defecto
        const rectMethodsHeight =
            methods.length > 1 ? methods.length * 40 : 40; // 40 es el valor mínimo por defecto
        ...
        return (
            <div className="umlClass">
                <svg width={widthValue} height={totalHeight} xmlns="http://www.w3.org/2000/svg">
                    ...
                    <rect
                        x="0"
                        y={rectAttributeHeight}
                        width={widthValue}
                        height={rectMethodsHeight}
                        fill="white"
                        stroke={borderColor}
                    />
                    ...
                    <line
                        x1="0"
                        y1={rectAttributeHeight}
                        x2={widthValue}
                        y2={rectAttributeHeight}
                        stroke="black"
                        strokeWidth="2"
                    />
